...
export class SKY extends plugin {
  ...
  async skyEncounterCheckIn(e) {
    ...
    if (!ITUE(USER_ID)) {
      ...
      REPLY = (e.adapter === 'QQBot') ? REPLY = [`# 第${(fs.readdirSync('plugins/Tlon-Sky/data/Sky签到')).length}位用户`, '> [设置昵称(昵称)]指令可设置昵称\n' + '> 示例：设置昵称小秋\n' + '> [设置头像(QQ号)]可设置为QQ头像\n' + '> 示例：设置头像114514'] : [segment.at(e.user_id), '\n设置昵称[昵称]指令可设置昵称\n示例：设置昵称小秋\n设置头像[QQ号]可设置为QQ头像\n示例：设置头像114514']
    }
    ...
    let Button = ''
    if (e.adapter === 'QQBot') Button = [[{ label: '设置昵称', data: '/设置昵称秋舞风' }, { label: '设置头像', data: '/设置头像1947425850' }]]
  ...
  async setNickname(e) {
    ...
    e.reply((e.adapter === 'QQBot') ? ['# 设置成功!', `> 您的昵称：**${NICKNAME}**`, Bot.Button([[{ label: '设置头像', data: '/设置头像1947425850' }]])] : `设置成功！\n您的昵称：${NICKNAME}`)
  }
  ...
  async setAvatar(e) {
    ...
    if (!avatar) { return e.reply((e.adapter === 'QQBot') ? ['# 未输入QQ号', '> 请附带QQ号，如：/设置头像3620060826', Bot.Button([[{ label: '设置头像', data: '/设置头像1947425850' }]])] : '请输入纯数字QQ号!') }

    if (avatar <= 10001) { e.reply((e.adapter === 'QQBot') ? ['# 输入QQ号低于10001', `> 默认设置3620060826为头像`, Bot.Button([[{ label: '设置头像', data: '/设置头像1947425850' }]])] : '输入QQ号不符合规范，默认设置3620060826为头像'); avatar = 3620060826 }
    ...
    e.reply((e.adapter === 'QQBot') ? ['> 设置成功！', segment.image(`https://q.qlogo.cn/g?b=qq&nk=${avatar}&s=640`)] : ['设置成功！', segment.image(`https://q.qlogo.cn/g?b=qq&nk=${avatar}&s=640`)])
  }
}
